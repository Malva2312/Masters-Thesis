...
from src.modules.model.standalone.vgg.vgg_model import VGGModel
from src.modules.loss_functions.vgg_loss_functions import VGGLossFunction

class PyTorchLightningVGGModel(pytorch_lightning.LightningModule):
    def __init__(self, config, experiment_execution_paths):
        super().__init__()
        self.config = config
        self.criterion = VGGLossFunction(
            config=self.config.vgg_config.criterion,
            experiment_execution_paths=experiment_execution_paths
        )
        self.labels = None
        self.model = VGGModel(config=self.config)
