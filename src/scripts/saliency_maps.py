...
def save_images(base_path, name_prefix, image_tensor, saliency_array):
    os.makedirs(base_path, exist_ok=True)
    original_path = join(base_path, f"{name_prefix}_original.png")
    zoom_path = join(base_path, f"{name_prefix}_zoom.png")
    saliency_overlay_path = join(base_path, f"{name_prefix}_saliency.png")
    ...
    ax.imshow(saliency_array, cmap='hot', alpha=0.5)
    ax.axis('off')
    ax.set_aspect('equal')
    plt.tight_layout()
    plt.savefig(saliency_overlay_path, dpi=300)
    plt.close()
...
def run_explainability(config):
    ...
    for fold_idx, test_dataloader in enumerate(kfold_dataloaders['test']):
        ...
        for idx, batch in enumerate(test_dataloader):
            ...
            # Saliency for fused and non-fused models
            saliency_fused = Saliency(lambda x: fused_model({'image': x}))
            saliency_non_fused = Saliency(lambda x: non_fused_model({'image': x}))
            ...
            for img_idx in range(batch_size):
                ...
                label = int(batch[1][img_idx].item())

                # Fused model Saliency
                image_tensor.requires_grad = True
                attributions_fused = saliency_fused.attribute(image_tensor, target=label)
                sal_fused_np = attributions_fused.squeeze().abs().cpu().numpy()
                sal_fused_np = (sal_fused_np - sal_fused_np.min()) / (sal_fused_np.max() - sal_fused_np.min() + 1e-8)
                ...
                save_images(img_dir, "sample", batch[0]['image'][img_idx], sal_fused_np)

                # Non-fused model Saliency
                image_tensor_non_fused = batch[0]['image'][img_idx].unsqueeze(0).to(device)
                image_tensor_non_fused.requires_grad = True
                attributions_non_fused = saliency_non_fused.attribute(image_tensor_non_fused, target=label)
                sal_non_fused_np = attributions_non_fused.squeeze().abs().cpu().numpy()
                sal_non_fused_np = (sal_non_fused_np - sal_non_fused_np.min()) / (sal_non_fused_np.max() - sal_non_fused_np.min() + 1e-8)
                sal_non_fused_path = join(img_dir, "sample_saliency_non_fused.png")
                original_np = batch[0]['image'][img_idx].squeeze().cpu().numpy()
                fig, ax = plt.subplots(figsize=(5, 5))
                ax.imshow(original_np, cmap='gray')
                ax.imshow(sal_non_fused_np, cmap='hot', alpha=0.5)
                ax.axis('off')
                ax.set_aspect('equal')
                plt.tight_layout()
                plt.savefig(sal_non_fused_path, dpi=300)
                plt.close()
